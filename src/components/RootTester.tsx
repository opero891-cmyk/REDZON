...
`;
  ...
  const headerActions = (
    <div className="flex items-center gap-1.5">
      <button
        ...
        className="px-2.5 py-1 rounded-lg bg-[#2DD4BF] hover:bg-[#14b8a6] text-[#09111D] font-bold text-[11px] flex items-center gap-1 transition-all shadow-md shadow-[#2DD4BF]/20"
      >
        <Download className="w-3 h-3" />
        <span className="hidden sm:inline">{isArabic ? 'تحميل السكريبت' : 'Download'}</span>
      </button>
      <button
        id="copy-script-btn"
        onClick={() => copyToClipboard(shellScript, 'full-script')}
        className="px-2.5 py-1 rounded-lg bg-[#09111D] hover:bg-[#16283B] text-slate-200 border border-slate-700 text-[11px] font-bold flex items-center gap-1 transition-colors"
      >
        {copiedScript === 'full-script' ? (
          <>
            <Check className="w-3 h-3 text-[#2DD4BF]" />
            <span className="text-[#2DD4BF] hidden sm:inline">{isArabic ? 'تم النسخ!' : 'Copied!'}</span>
          </>
        ) : (
          <>
  ...
  return (
    <CollapsibleCard
      id="root-tester-section"
      title={isArabic ? 'دليل واختبار التنفيذ الحقيقي على الهاتف' : 'Real Hardware Verification & Live Script'}
      subtitle={isArabic ? 'طريقة تجربة هذه الأوامر والتأكد من فعاليتها المباشرة على هاتفك' : 'How to execute and verify these exact sysfs commands live on your rooted Android phone'}
      icon={Smartphone}
      badge={headerBadge}
    ...
    >
      <div className="space-y-4">
        ...
        <div className="bg-[#09111D] border border-slate-800 rounded-xl p-3.5">
